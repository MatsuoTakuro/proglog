...
func setupTest(t *testing.T, fn func(*Config)) (
	...
	if *debug {
		metricsLogFile, err := os.CreateTemp("observability_output/", "metrics-*.log")
		require.NoError(t, err)
		t.Logf("metrics log file: %s", metricsLogFile.Name())

		tracesLogFile, err := os.CreateTemp("observability_output/", "traces-*.log")
		require.NoError(t, err)
	...
	return rootCli, nobodyCli, cfg, func() {
		...
		if telemetryExporter != nil {
			// take some time to flush data to disks
